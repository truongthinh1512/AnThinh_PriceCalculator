...
function App() {
    // --- STATE ---
    const [activeTab, setActiveTab] = useState('calculator'); // 'calculator' | 'catalog' | 'customer'
    const [catalogTab, setCatalogTab] = useState('lamination'); // 'lamination' | 'winding' | 'accessory'
    const [customerTab, setCustomerTab] = useState('list'); // 'list'
    ...
    const fetchCatalog = async () => {
        try {
            const [lamRes, windRes, accRes, txRes, cusRes] = await Promise.all([
    ...
    const calculateWindingCost = (wUsage) => {
        ...
        // Remove turnLength logic. Use direct weight input.
        ...
        if (newTransformer.type === 'SQUARE') {
            const payload = {
                ...
                windings: result.windings.map(w => ({
                    ...
                    type: w.type // Send type to backend
                })),
            ...
            try {
                ...
                if (editingId) {
                    ...
                    console.log("Saving new transformer with payload:", payload);
    ...
    const addWinding = () => {
        setNewTransformer(prev => ({
            ...prev,
            windings: [...prev.windings, { specId: '', weight: 0.0, type: 'PRIMARY' }]
        }));
    };
        ...
        <div className="container mx-auto p-4 min-h-screen font-sans text-gray-800 pb-20">
            ...
            <header className="mb-8 flex flex-col md:flex-row justify-between items-center gap-4 border-b pb-4 bg-white p-4 rounded-lg shadow-sm">
                ...
                <nav className="flex bg-gray-100 p-1 rounded-lg">
                    ...
                    <button onClick={() => setActiveTab('customer')} className={`flex items-center gap-2 px-6 py-2 rounded-md transition-all duration-200 text-sm font-medium ${activeTab === 'customer' ? 'bg-white text-blue-600 shadow-sm' : 'text-gray-500 hover:text-gray-700'}`}>
                        <span className="w-4 h-4 font-bold">KH</span> Khách Hàng
                    </button>
                </nav>
                <div>
            ...
            {/* CALCULATOR SCCONNECTION SUCCESS */}
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-12 gap-8">
                        ...
                        <div className="md:col-span-12 space-y-8">
                            {/* LIVE PREVIEW (Improved Layout) */}
                            {lastCalculated && (
                                <div className="bg-white rounded-xl shadow-lg border border-blue-100 overflow-hidden animate-in zoom-in-95 duration-300">
                                    {/* Header Section */}
                                    <div className="bg-gradient-to-r from-slate-800 to-slate-900 px-8 py-6 text-white flex flex-col md:flex-row justify-between items-center gap-4">
                                        <div>
                                            <h2 className="font-bold text-2xl flex items-center gap-2 text-blue-400">
                                                <Save className="w-6 h-6" /> Bảng Tính Chi Tiết
                                            </h2>
                                            <p className="text-gray-400 text-sm mt-1">{lastCalculated.description || lastCalculated.name}</p>
                                        </div>
                                        <div className="text-right">
                                            <div className="text-gray-400 text-xs uppercase tracking-wider mb-1">TỔNG THÀNH TIỀN</div>
                                            <div className="font-mono font-bold text-4xl text-green-400 drop-shadow-sm">
                                    ...
                                    <div className="p-0">
                                        <table className="w-full text-sm text-left">
                                            <thead className="bg-gray-50 text-gray-500 uppercase text-xs font-semibold tracking-wider border-b">
                                                <tr>
                                                    <th className="px-6 py-4 w-1/12 text-center">#</th>
                                                    <th className="px-6 py-4 w-4/12">Hạng Mục</th>
                                                    <th className="px-6 py-4 w-3/12 text-right">Chi Tiết</th>
                                                    <th className="px-6 py-4 w-2/12 text-right">Đơn Giá</th>
                                                    <th className="px-6 py-4 w-2/12 text-right font-bold text-gray-700">Thành Tiền</th>
                                                </tr>
                                            </thead>
                                            <tbody className="divide-y divide-gray-100">
                                                {/* 1. CORE */}
                                                <tr className="hover:bg-blue-50/30 transition-colors">
                                                    <td className="px-6 py-4 text-center font-bold text-blue-600">1</td>
                                                    <td className="px-6 py-4">
                                                        <div className="font-bold text-gray-800">{lastCalculated.core.description}</div>
                                                        <div className="text-xs text-blue-500 font-medium bg-blue-50 inline-block px-2 py-0.5 rounded mt-1">LÕI TỪ (CORE)</div>
                                                    </td>
                                                    <td className="px-6 py-4 text-right">
                                                        <span className="font-mono">{lastCalculated.core.weight} kg</span>
                                                    </td>
                                                    <td className="px-6 py-4 text-right text-gray-500">
                                                       {/* Unit price for core is tricky to display simply here as it may be sum of multiple things, keeping simple */}
                                                       -
                                                    </td>
                                                    <td className="px-6 py-4 text-right font-bold text-gray-800">
                                                        {formatMoney(lastCalculated.core.totalCost)}
                                                    </td>
                                                </tr>

                                                {/* 2. WINDINGS */}
                                                {lastCalculated.windings.map((w, idx) => (
                                                    <tr key={`w-${idx}`} className="hover:bg-orange-50/30 transition-colors">
                                                        <td className="px-6 py-4 text-center font-bold text-orange-600">{idx + 2}</td>
                                                        <td className="px-6 py-4">
                                                            <div className="font-bold text-gray-800">{w.name}</div>
                                                            <div className="text-xs text-orange-500 font-medium bg-orange-50 inline-block px-2 py-0.5 rounded mt-1">DÂY {w.type === 'PRIMARY' ? 'SƠ CẤP' : w.type === 'SECONDARY' ? 'THỨ CẤP' : 'BIẾN ÁP PHỤ'}</div>
                                                        </td>
                                                        <td className="px-6 py-4 text-right">
                                                            <span className="font-mono">{w.calculatedWeight} kg</span>
                                                        </td>
                                                        <td className="px-6 py-4 text-right text-gray-500">
                                                            {/* We need to fetch unit price from spec list or store it in performCalculations. Assuming performCalculations has it implicitly via cost/weight or we add it there. 
                                                                Ideally, modify performCalculation to include unitPrice. For now let's calc it or just show total. 
                                                            */}
                                                            {formatMoney(w.cost / (w.calculatedWeight || 1))}/kg
                                                        </td>
                                                        <td className="px-6 py-4 text-right font-bold text-gray-800">
                                                            {formatMoney(w.cost)}
                                                        </td>
                                                ...
                                                {/* 3. ACCESSORIES */}
                                                {lastCalculated.accessories.map((a, idx) => (
                                                    <tr key={`a-${idx}`} className="hover:bg-purple-50/30 transition-colors">
                                                        <td className="px-6 py-4 text-center font-bold text-purple-600">{lastCalculated.windings.length + idx + 2}</td>
                                                        <td className="px-6 py-4">
                                                            <div className="font-bold text-gray-800">{a.name}</div>
                                                            <div className="text-xs text-purple-500 font-medium bg-purple-50 inline-block px-2 py-0.5 rounded mt-1">PHỤ KIỆN</div>
                                                        </td>
                                                        <td className="px-6 py-4 text-right">
                                                            <span className="font-mono">{a.quantity}</span>
                                                        </td>
                                                        <td className="px-6 py-4 text-right text-gray-500">
                                                            {formatMoney(a.cost / (a.quantity || 1))}
                                                        </td>
                                                        <td className="px-6 py-4 text-right font-bold text-gray-800">
                                                            {formatMoney(a.cost)}
                                                        </td>
                                            ...
                                            <tfoot className="bg-slate-50 border-t border-slate-200">
                                                <tr>
                                                    <td colSpan="4" className="px-6 py-4 text-right font-bold text-gray-600 uppercase text-xs tracking-wider">Tổng cộng</td>
                                    ...
                                    <div className="bg-gray-50 px-8 py-4 border-t flex justify-end gap-3">
                                        <button onClick={() => setLastCalculated(null)} className="px-4 py-2 text-gray-500 hover:text-gray-700 font-medium text-sm">Đóng xem trước</button>
                                        <button onClick={() => document.getElementById('transformer-form').requestSubmit()} className="bg-blue-600 hover:bg-blue-700 text-white px-6 py-2 rounded-lg font-bold shadow-sm text-sm">
                                            Lưu Kết Quả Này
                                        </button>
                                    </div>
                                </div>
                             ...
                             <form id="transformer-form" onSubmit={handleCalculateAndSave} className="space-y-8">
                                ...
                                <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                                    ...
                                    <div>
                                        ...
                                        <select value={newTransformer.customerId || ''} onChange={e => setNewTransformer(prev => ({ ...prev, customerId: e.target.value }))} className="w-full rounded-lg border-gray-300 shadow-sm focus:border-blue-500 focus:ring-blue-500 p-3 border">
                                ...
                                <div className="bg-gray-50 rounded-xl p-6 border border-gray-200">
                                    ...
                                    <div className="mb-4 bg-orange-50 p-3 rounded-lg border border-orange-100 hidden">
                                        {/* REMOVED Turn Logic based on user request */}
                                    </div>
                                    ...
                                    <div className="space-y-3">
                                        {newTransformer.windings.map((w, idx) => (
                                            <div key={idx} className="flex gap-3 items-start bg-white p-3 rounded-lg border border-gray-200 shadow-sm">
                                                <div className="w-32">
                                                    <label className="text-xs font-bold text-gray-500 mb-1 block">Loại Cuộn</label>
                                                    <select value={w.type} onChange={e => updateWinding(idx, 'type', e.target.value)} required className="w-full text-sm rounded-md border-gray-300 p-2 border bg-gray-50">
                                                        <option value="PRIMARY">Sơ cấp</option>
                                                        <option value="SECONDARY">Thứ cấp</option>
                                                        <option value="LIGHTING">Đèn (Phụ)</option>
                                                    </select>
                                                </div>
                                                <div className="flex-1">
                                                ...
                                                <div className="w-32">
                                                    <label className="text-xs font-bold text-gray-500 mb-1 block">Khối Lượng (kg)</label>
                                                    <input value={w.weight} onChange={e => updateWinding(idx, 'weight', e.target.value)} type="number" step="0.01" className="w-full text-sm rounded-md border-gray-300 p-2 border" placeholder="kg" />
                                                </div>
            ...
            {activeTab === 'customer' && (
                <div className="animate-in fade-in duration-300 space-y-6">
                    <form onSubmit={saveCustomer} className="grid grid-cols-1 md:grid-cols-4 gap-4 bg-white p-6 rounded-xl shadow-sm border border-gray-200">
                        <input value={newCustomer.name} onChange={e => setNewCustomer(prev => ({ ...prev, name: e.target.value }))} type="text" placeholder="Tên Khách Hàng" required className="rounded-md border-gray-300 p-2 border"/>
                        <input value={newCustomer.phoneNumber} onChange={e => setNewCustomer(prev => ({ ...prev, phoneNumber: e.target.value }))} type="text" placeholder="Số điện thoại" className="rounded-md border-gray-300 p-2 border"/>
                        <input value={newCustomer.address} onChange={e => setNewCustomer(prev => ({ ...prev, address: e.target.value }))} type="text" placeholder="Địa chỉ" className="rounded-md border-gray-300 p-2 border"/>
                        <div className="flex gap-2">
                             <input value={newCustomer.note} onChange={e => setNewCustomer(prev => ({ ...prev, note: e.target.value }))} type="text" placeholder="Ghi chú" className="flex-1 rounded-md border-gray-300 p-2 border"/>
                             <button type="submit" className="bg-blue-600 text-white rounded-md w-12 hover:bg-blue-700 font-bold"><Plus className="w-5 h-5 mx-auto" /></button>
                        </div>
                    </form>
                            ...
                            <tbody>
                                {customers.map(c => (
                                    <tr key={c.id} className="bg-white border-b hover:bg-gray-50">
                                        ...
                                        <td className="px-6 py-4 italic">{c.note}</td>
                                        <td className="px-6 py-4 text-right">
                                            <button onClick={() => deleteCustomer(c.id)} className="text-gray-400 hover:text-red-600"><Trash2 className="w-4 h-4" /></button>
                                        </td>
                                    </tr>
                                ))}
                            </tbody>
